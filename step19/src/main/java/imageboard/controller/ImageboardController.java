...
public class ImageboardController {
	...
	// 파라미터 MultipartFile img는 <input type="file" name="img"> 
	// 태그의 name과 일치해야 한다.
	...
	public ModelAndView imageboardWrite(MultipartFile img, HttpServletRequest request) {
		String filePath = "D:/android_201910/spring/workspace/"
						+ "step19/src/main/webapp/storage";
		...
		try {
			...
			FileCopyUtils.copy(img.getInputStream(), 
		...
		int imagePrice = 
			Integer.parseInt(request.getParameter("imagePrice"));
		int imageQty  = 
			Integer.parseInt(request.getParameter("imageQty"));
